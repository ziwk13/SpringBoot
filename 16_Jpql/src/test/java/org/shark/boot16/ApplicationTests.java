...
class ApplicationTests {
  ...
  void multiRowQueryTest() {
	...
	@Test
	@DisplayName("1. logLevel의 중복을 제거하여 조회하세요.")
	void distinctLogLevel() {
	  String jpql = "SELECT DISTINCE al.logLevel FROM AccessLog al";
	  TypedQuery<String> typedQuery = em.createQuery(jpql, String.class);
	...
	@Test
	@DisplayName("logId가 1에서 5인 엔티티를 조회하세요.")
	void betweenAndOperatorTest() {
	  ...
	  TypedQuery<AccessLog> typeQuery = em.createQuery(jpql, AccessLog.class)
	                                      .setParameter("begin", begin)
	                                      .setParameter("end", end);
	  Stream<AccessLog> stream = typeQuery.getResultStream();
	  stream.forEach(accessLog -> log.info("{}", accessLog));
	}
	// 3. logId가 1 또는 5인 엔티티를 조회하세요.
	@Test
	@DisplayName("logId가 1 또는 5인 엔티티를 조회하세요.")
	void inOperatorTest() {
	  
	}
	
	// 4. logLevel에 "USER"가 포함된 엔티티를 조회하세요.
	...
	void groupFunctionTest() {
	  // COUNT 함수
	  // 1. 반환 타입이 Long
	  // 2. 데이터가 없는 경우 0을 반환 (원래는 null JPA가 0으로 변환) 반환 타입을 long으로 설정 해도 된다.
	  // 3. COUNT 함수 외 통계함수(예: SUM)는 데이터가 없는 경우 null 반환
	  ...
	  // COALESCE 함수
	  // 1. SQL 표준 함수 ( jPQL 지원 가능)
	  // 2. 전달된 인자들을 순서대로 조회하여 null이 아닌 첫 번째 값을 반환 한다.
	  // 3. 모든 값이 null이면 null을 반환
	  ...
	  String jpql4 = "SELECT AVG(al.log) FROM AccessLog al";
	...
	void groupByTest() {
	  Long groupCount = 2L;
	  String jpql = "SELECT al.logLevel, COUMT(al.logId) FROM AccessLog al GROUP BY al.logLevel HAVING COUNT(al.log)";
